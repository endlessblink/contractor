...
import { validateForm } from './shared/form-validation.mjs';
...
const runtimeSkills = initRuntimeSkills();
console.log(`Loaded runtime skills: ${runtimeSkills.length} editable Markdown skills from ${USER_SKILLS_DIR}`);
...
**לעולם אל תכתוב הצעת מחיר, חוזה, הזמנת עבודה או קורות חיים מלאים בתוך הצ'אט.** המסמכים נוצרים דרך הטופס בלשונית "יצירת מסמך".
...
כללים:
- docType: "quote" | "order" | "contract" | "cv"
...
- notes = הערות **ספציפיות לפרויקט בלבד** (למשל: גישה לשרת הלקוח, תלות בספק צד-שלישי, הנחה חד-פעמית). **לא** תנאים משפטיים — אלה מגיעים מהסעיפים המובנים.
...
- cvData הוא מקור האמת לתוכן קורות החיים.
...
## מקור האמת לתנאים
- **הסעיפים המובנים (מאגר הסעיפים) הם מקור האמת היחיד לכל תוכן משפטי/תנאים** — מע"מ, תוקף הצעה, תנאי תשלום, מקדמה, חשבונית, אחריות, קניין רוחני, ביטול וכו'.
- שדה "הערות כלליות" (notes) שמור **אך ורק** להערות ספציפיות לפרויקט.
- **לעולם אל תחזור על תוכן של סעיף בהערות ואל תסתור אותו.** אם תוכן שייך לסעיף — אל תכתוב אותו ב-notes. המערכת תסיר אוטומטית הערות שחוזרות על סעיפים.
...
- **אל תוסיף משפטי שיווק/נימוסין** כמו "אשמח להתאים", "נשמח לעמוד לרשותכם", "בואו נדבר". הערות צריכות להיות עובדתיות וספציפיות לפרויקט בלבד — **לא** חזרה על תנאים שכבר מופיעים בסעיפים (תוקף, מע"מ, תשלום).
...
6. **אל תשתמש ב-FORM_UPDATE לשינוי סוג מסמך** — רק FORM_DATA מחליף את הטופס כולו`;
  ...
  prompt += buildRuntimeSkillsPromptSection({ includeAll: true });
...
// ─── Runtime Document Skills API ──────────────────────────────────────────────

app.get('/api/runtime-skills', (_req, res) => {
  try {
    const skills = loadRuntimeSkills().map(skill => ({
      id: skill.id,
      name: skill.name,
    ...
    res.json({ skills, directory: USER_SKILLS_DIR });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

app.get('/api/runtime-skills/:id', (req, res) => {
  try {
    const skill = loadRuntimeSkill(req.params.id);
...
app.put('/api/runtime-skills/:id', (req, res) => {
  try {
    ...
    const status = err.message.includes('Invalid skill') ? 400 : 500;
...
app.post('/api/chat', async (req, res) => {
  ...
  if (formContext) {
    ...
    if (formContext.docType) {
      const docTypeNames = { quote: 'הצעת מחיר', order: 'הזמנת עבודה', contract: 'חוזה', cv: 'קורות חיים' };
...
app.post('/api/generate-document', async (req, res) => {
  try {
    ...
    const docTypeMap = { quote: 'quote', order: 'workOrder', contract: 'contract', cv: 'cv' };
    const paymentLabels = {
      two: [
...

{
  ...
  "documentType": "quote" | "contract" | "workOrder" | "cv",
...
app.post('/api/recommend-clauses', async (req, res) => {
  try {
    ...
    const docTypeKey = docType === 'order' ? 'workOrder' : docType === 'contract' ? 'contract' : docType === 'cv' ? 'cv' : 'quote';
    const docTypeNames = { quote: 'הצעת מחיר', order: 'הזמנת עבודה', contract: 'חוזה', cv: 'קורות חיים' };
...
// ─── Deterministic form validation (approve-time checks, no LLM) ──────────────
...
app.post('/api/validate-form', (req, res) => {
  try {
    ...
    const result = validateForm(formContext || {}, clausesDb);
    res.json(result);
  } catch (err) {
    console.error('[validate-form] error:', err.message);
    res.status(500).json({ error: err.message });
  }
});
